module reg_n_bit #(parameter N = 8)
    ...
    // ANY signal in sensitivity list that changes will trigger re-evaluation of always block
        // since we want the register to update on the posedge clk, it is in list
        // don't put enable in the list bc that means register values update if EITHER
        // posedge clk OR the enable edge changes. 
            // enable is a condition inside the always block that is checked on posedge clk 
        begin
            if (reset)
                Q <= 0;
            else if (enable)
